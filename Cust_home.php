...
class Cust_home extends CI_Controller
{
	...
	function forgot()
	{
		if($_REQUEST)
		{
			$email = App_string_encrypt($_REQUEST['email']);	//mysqli_real_escape_string
			$flag = $_REQUEST['flag'];
			...
			if($result != NULL || $result > 0)
			{
				...
				$post_data=array('Pwd_set_code' => $Set_pwd_code);
				$result2 = $this->Igain_model->update_password($Company_id,$result->Enrollement_id,$post_data);
				
				$Email_content = array(
					'Password' => $result->User_pwd,
					'Notification_type' => 'Request For Password',
					'Pwd_set_code' => $Set_pwd_code,
				...
				if($flag == 2)
				{
					...
					$result['eml']='1';
					 ...
					 $this->session->set_flashdata("success_code",'Login Credentials are sent to your email...please check it !!');
				}
				else
				{
					// echo '2';
	...
	{
		/* echo"<br> Home cust_logged_in";			
		print_r($this->session->set_userdata('cust_logged_in'));
		echo"<br>cust_logged_in";
		die; */
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			// $Company_Details= $this->Igain_model->get_company_details($session_data['Company_id']);
			$data['Profile_complete_flag']=$data['Company_Details']->Profile_complete_flag;
			$data['Profile_complete_points']=$data['Company_Details']->Profile_complete_points;
			...
			$f=0;
			$data['LatestOffers'] = $this->Igain_model->Fetch_customer_latest_offer($data['enroll'],$data['Company_id']);
			...
			/*******************AMIT 25-07-2017--END--------Enroll_details--******************/
	...
	function profile()
	{
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$exp=explode($dial_code->phonecode,App_string_decrypt($Enroll_details->Phone_no));
			$data['phnumber'] = $exp[1];
	...
	public function updateprofile()
	{
		...
		/* 	$config['upload_path'] = '../uploads/'; 
			...
			else if($this->form_validation->run() == TRUE)
			{
				...
				$User_email_id = strip_tags($this->input->post('userEmailId'));
				
				if (!filter_var($User_email_id, FILTER_VALIDATE_EMAIL)) {
				  $this->session->set_flashdata("error_code","Please Enter Valid Email Address");
				  redirect('Cust_home/profile');
				}
				$userEmailId=App_string_encrypt($User_email_id);
				
				if($userEmailId !=$Enroll_details->User_email_id)
				{
					$EmailIdresult = $this->Igain_model->Check_EmailID($userEmailId,$Company_id);
					if($EmailIdresult > 0)
					{
						$this->session->set_flashdata("error_code","Email Id Already Exist!");
						redirect('Cust_home/profile');
					}
				}
			
				$phno= strip_tags($this->input->post("phno"));
				
				$checkPhone = is_numeric($phno);
				if($checkPhone !=1)
				{
				...
				$userphno=App_string_encrypt($phno);
				if($userphno !=$Enroll_details->Phone_no)
				{
					$CheckPhone_number=$this->Igain_model->CheckPhone_number($userphno,$Company_id);
					if($CheckPhone_number > 0)
					{
						$this->session->set_flashdata("error_code","Phone Number Already Exist");
						redirect('Cust_home/profile');
					}
				}
				
				$StateArray = array();
				$data['States_array2'] = $this->Igain_model->Get_states($Country);
				foreach($data['States_array2'] as $row){
					$StateArray[] = $row->id;
				}
				if (!in_array(strip_tags($this->input->post('state')), $StateArray)){
					$this->session->set_flashdata("error_code","Please Select Valid State");
					redirect('Cust_home/profile');
				}
				
				$data['City_array2'] = $this->Igain_model->Get_cities(strip_tags($this->input->post('state')));

				$CityArray = array();
				foreach($data['City_array2'] as $row){
					$CityArray[] = $row->id;
				}
				if (!in_array(strip_tags($this->input->post('city')), $CityArray)){
					$this->session->set_flashdata("error_code","Please Select Valid City");
					redirect('Cust_home/profile');
				}
				$Gender =strip_tags($this->input->post('Sex'));
				
				$GenderStatus = array('Male','Female','');
				if (!in_array($Gender, $GenderStatus)){
					$this->session->set_flashdata("error_code","Please Select Valid Gender");
					redirect('Cust_home/profile');
				}
				
				$Marital_status = strip_tags($this->input->post('Marital_status'));
				
				$MaritalStatus = array('Single','Married','');
				if (!in_array($Marital_status, $MaritalStatus)){
					$this->session->set_flashdata("error_code","Please Select Valid Marital Status");
					redirect('Cust_home/profile');
				}
			...
			// $result = preg_replace('/[^A-Za-z0-9\-]/', '', $string);
				...
				$First_name = preg_replace('/[^A-Za-z]/', ' ', $firstName);
				
				$middleName = strip_tags($this->input->post('middleName'));
				$Middle_name = preg_replace('/[^A-Za-z]/', ' ', $middleName);
				
				$lastName = strip_tags($this->input->post('lastName'));
				$Last_name = preg_replace('/[^A-Za-z]/', ' ', $lastName);
				
				$district = strip_tags($this->input->post('district'));
				$Cust_district = preg_replace('/[^A-Za-z]/', ' ', $district);
			
				$zip = strip_tags($this->input->post('zip'));
				$Zipcode = preg_replace('/[^0-9]/', '', $zip);
				
				$Profession = strip_tags($this->input->post('Profession'));
				$Qualification = preg_replace('/[^A-Za-z]/', ' ', $Profession);
			
				$Experience = strip_tags($this->input->post('Experience'));
				$Experience1 = preg_replace('/[^0-9]/', '', $Experience);
				
				$post_data = array(
						'First_name' => $First_name,
						'Middle_name' => $Middle_name,
						'Last_name' => $Last_name,
						'District' => $Cust_district,
						'Zipcode' => $Zipcode,
						...
						'Current_address' => App_string_encrypt(strip_tags($this->input->post('currentAddress'))),
			...
			/************************************************/
				if($result == true)
				{
					...
					$this->session->set_flashdata("error_code","Profile Updated Successfuly. Your profile ".$Customer_profile_status."% Complete !!");
					...
					if($Customer_profile_status =='100' && $Customer_details->Profile_complete_flag=='0' && $Company_Details->Profile_complete_flag=='1')
					{
						...
						$this->session->set_flashdata("error_code","Profile Updated Successfuly. Your profile ".$Customer_profile_status."% Complete !!<br>Congrats, You have received ".round($Company_Details->Profile_complete_points)."  Points !!!");
					}
					/*********************Nilesh igain Log Table change 27-06-207*************************/
						$Enroll_details1 = $this->Igain_model->get_enrollment_details($session_data['enroll']);
						$opration = 2;
						...
						$Todays_date=date("Y-m-d");
						$firstName = $Enroll_details1->First_name;
						$lastName = $Enroll_details1->Last_name;
						$LogginUserName = $Enroll_details1->First_name.' '.$Enroll_details1->Last_name;
					...
					if($userEmailId !=$User_email_id)
					{
						$this->session->set_flashdata("error_code","logout");
						redirect('Cust_home/profile');
	...
	public function update_promocode()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			else
			{
				...
				$promo_code = preg_replace('/[^A-Za-z0-9\-]/', '', $promo_code);
				if($promo_code=="" ||$promo_code==null)
				{
					$this->session->set_flashdata("error_promo","Please Enter Promo Code");
					redirect('Cust_home/promocode');
				}
	...
	function My_Vouchers()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data["NotificationsCount"] = $this->Igain_model->Fetch_Open_Notification_Count($data['enroll'],$data['Company_id']);
			$data["NotificationsDetails"] = $this->Igain_model->FetchNotificationDetails($data['enroll'],$data['Company_id']);
			$data["My_Vouchers"] = $this->Igain_model->Fetch_My_Vouchers($data['enroll']);
			// die;
			$Country_details = $this->Igain_model->get_dial_code($data['Country_id']);
			
			$data['Symbol_of_currency'] = $Country_details->Symbol_of_currency;	
			
			$this->load->view('home/My_Vouchers', $data);
		}
		else
		{
			redirect('login', 'refresh');
	...
	function logout()
	{
		...
		if($_REQUEST['ch']==1)
		{
			$this->session->set_flashdata("success_code",'Password has Changed Successfully.. Please login again.. !!');
		}
		redirect('login', 'refresh');
	}
	function mailbox()
	...
	function mystatement()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			{
				if(is_numeric($_REQUEST["Show"])==1)
				{
					$data["Transaction_Reports"]=array_slice($_SESSION["Transaction_Reports"], 0,$_REQUEST["Show"]);
					$_SESSION["Show"] = $_REQUEST["Show"];
				}
				/* else
				{
					$data["Transaction_Reports"]=array_slice($_SESSION["Transaction_Reports"], 0);
				} */
				
			}
			
			if($_REQUEST != NULL && !isset($_REQUEST["Show"]))
			{
				// echo strip_tags($_REQUEST['startDate']);
				
				// die;
				$data["Transaction_Reports"] = $this->Igain_model->Fetch_Transaction_Detail_Reports($session_data['Company_id'],$_REQUEST['startDate'],$_REQUEST['endDate'],strip_tags($_REQUEST['Merchant']),strip_tags($_REQUEST['Trans_Type']),strip_tags($_REQUEST['Report_type']),$data['enroll'],$data['Card_id'],$_REQUEST['Redeemption_report'],'','');
				
				$_SESSION['Transaction_Reports']=$data["Transaction_Reports"];
				$_SESSION['startDate']=$_REQUEST['startDate'];
				$_SESSION['endDate']=$_REQUEST['endDate'];
				$_SESSION['Redeemption_report']=strip_tags($_REQUEST['Redeemption_report']);
				$_SESSION['Report_type']=strip_tags($_REQUEST['Report_type']);
				$_SESSION['Merchant']=strip_tags($_REQUEST['Merchant']);
				$_SESSION['Trans_Type']=strip_tags($_REQUEST['Trans_Type']);
	...
	function insertauctionbidding()
	{
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			foreach($Auction_array as $Auction)
			{
					...
					$Auction_Max_Bid_val[ $Auction['Auction_id'] ] = $this->Igain_model->Auction_Max_Bid_Value($Auction['Auction_id'],$data['Company_id']);
					$data["Auction_Max_Bid_val"] = $Auction_Max_Bid_val;Enroll_details
			} */
			...
			{
				// echo $_POST['current_bal'];die;
				
				$Auction_Bid_Value_Validate['Bid_value'] = $this->Igain_model->Fetch_Auction_Max_Bid_Value(App_string_decrypt($_POST['auctionID']),$data['Company_id']);
				...
				// echo 'bidval '.($_POST['bidval']);
				
				if(($_POST['bidval'] >= round($auction_val)) && ($_POST['bidval'] <= $_POST['current_bal']))
				{
					// echo '<br>auction_val '.($auction_val);die;
					$result = $this->Igain_model->insert_auction_bidding($Super_Seller_enroll,$data['enroll'],$data['Company_id'],App_string_decrypt($_POST['auctionID']),($_POST['bidval']),App_string_decrypt($_POST['Prize']));
					if($result == true)
					{
						$result  = array();
						$result['res']='1';
					/*********************Nilesh igain Log Table change 27-06-207*************************/
						$Member_Enrollment_id = $data['enroll'];
	...
	public function check_email_id()
	{
		...
		$userEmailId=App_string_encrypt($this->input->post("userEmailId"));
		$result = $this->Igain_model->Check_EmailID($userEmailId,$this->input->post("Company_id"));
	...
	public function checkoldpassword()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$result = $this->Igain_model->Check_Old_Password($old_Password,$Company_id,$Enrollment_id);
			if($result == "")
			{
				$result  = array();
				$result['res']='1';
	...
	public function checkoldpin()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$result = $this->Igain_model->Check_Old_Pin($this->input->post("old_pin"),$Company_id,$Enrollment_id);
			if($result == "")
			{
				$result  = array();
				$result['res']='1';
	...
	public function changepassword()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$Enroll_password = App_string_decrypt($Enroll_details->User_pwd);
			
			$old_Password=$this->input->post('old_Password');
			$confirm_Password=$this->input->post('confirm_Password');
			$new_Password=$this->input->post('new_Password');
			
			if($Enroll_password != $old_Password)
			{
				$this->session->set_flashdata("error_code","Old Password is Incorrect.. Please try again");
				$this->output->set_output(0);
				exit;	
			}
			if($old_Password==$new_Password)
			{
				$this->session->set_flashdata("error_code","Old Password is same with New Password.. Please try again");
				$this->output->set_output(0);
				exit;
			}
			if($confirm_Password!=$new_Password)
			{
				$this->session->set_flashdata("error_code","New Password and Confirm Password don't match..Please try again");
				$this->output->set_output(0);
				exit;
			}
			...
			if($password_len < 8)
			{	
				$this->session->set_flashdata("error_code","Password should be minimum of 8 characters with one number, one special, one upper and one lower case letter");	
				$this->output->set_output(0);		
				exit;				
			}
			...
			$result = $this->Igain_model->Change_Old_Password($old_Password,$Company_id,$Enrollment_id,App_string_encrypt($new_Password));
			if($result)
			{
			...
			if($Enroll_pinno == $Old_Pin)
			{
				if($newpin == $confirm_pin)
				{
					...
					if($result_pin == true)
					{
						...
						$result_pin  = array();
						$result_pin['pwd']='1';
						$this->session->set_flashdata("error_code","Pin has Changed Successfuly");
				...
				{
					$this->session->set_flashdata("error_code","New Pin and Confirm Pin don't match, Please Try Again");
				}
			}
			else
			{
				$this->session->set_flashdata("error_code","Old Pin is Incorrect, Please Try Again");
			}
		}
		else
		{
		...
		if($this->session->userdata('cust_logged_in'))
		{
			...
			if($get_pin->pinno != "" || $get_pin->pinno != 0 )
			{
				...
				/*********************Nilesh igain Log Table change 27-06-207*************************/
					...
					$result_log_table = $this->Igain_model->Insert_log_table($Company_id,$Enrollment_id,$User_email_id,$LogginUserName,$Todays_date,$what,$where,$userid,$opration,$opval,$firstName,$lastName,$Enrollment_id);
				/**********************igain Log Table change 27-06-2017 *************************/
				$this->session->set_flashdata("error_code","Your Pin sent on your registred email ID Successfully");
				$this->output->set_output(1);
			}
			else
			{
				$this->session->set_flashdata("error_code","Send Pin  on your registred Email is Un-successful...Please Contact Administrator");
				$this->output->set_output(0);
			}
		}
		else
	...
	public function export()
	{
		...
		/* 	if($Report_type == '1')
			...
			$Export_file_name = $Today_date."_Member_report";
			$data["Transaction_Reports"] = $_SESSION["Transaction_Reports"];
			$data["From_date"] = $_SESSION['startDate'];
			$data["end_date"] = $_SESSION['endDate'];
			$data["Report_type"] = $_SESSION['Report_type'];
			$data["Redeemption_report"] = $_SESSION['Redeemption_report'];
			/* $data["Report_date"] = $Today_date;
			$data["Report_type"] = $Report_type;
			$data["From_date"] = $_SESSION['startDate'];
			$data["end_date"] = $_SESSION['startDate'];
			...
			if($pdf_excel_flag == 1)
			{
				...
				foreach($_SESSION["Transaction_Reports"][0] as $key => $field)
				{
					$fields[]=$key;
				}
				$col = 0;
				...
				$row = 2;
				foreach($_SESSION["Transaction_Reports"] as $data1)
				{
					$col = 0;
					foreach ($fields as $field)
					{
	...
	function contactus()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			if ($this->form_validation->run() == FALSE)
			{
				$this->session->set_flashdata("Contactus","Please Fill Up Required Fileds");
			...
			else if($this->form_validation->run() == TRUE)
			{
				...
				$membership_id =  $session_data['Card_id'];
				$contact_subject =  strip_tags($this->input->post('contact_subject'));
				$contactus_SMS =  strip_tags($this->input->post('offerdetails'));
				
				$contact_subject_list = array('1','2','3','4'); 
				
				if (!in_array($contact_subject, $contact_subject_list))
				{
					$this->session->set_flashdata("Contactus","Please Select Valid Subject");
					redirect('Cust_home/contactus');
				}
				...
				else if($contact_subject == 4 && $contact_subject != "" && $contactus_SMS != "")
				{
					...
					if($result == true && $result1 == true)
					{
						...
						$Excecative_email=App_string_decrypt($Enroll_details1->User_email_id);
						$Excecative_contact_no=App_string_decrypt($Enroll_details1->Phone_no);
						$Email_content = array(
								'Today_date' => $today,
								'Cust_name' => $Cust_name,
								...
								'Excecative_contact_no' => $Excecative_contact_no,
	...
	function transferpoints()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			$data['Country_id'] = $session_data['Country_id'];
			$Company_id = $session_data['Company_id'];
			$Login_Enrollement_id = $session_data['enroll'];
			$Login_Membership_id = $session_data['Card_id'];
			...
			$Login_Current_balance = $data["Enroll_details"]->Current_balance;
			$Current_point_balance = ($data["Enroll_details"]->Total_balance-$data["Enroll_details"]->Debit_points);			
			...
			if($_POST != NULL)
			{
				...
				if ($this->form_validation->run() == FALSE)
				{
					$this->session->set_flashdata("transfer","Please Fill Up Required Fileds");
					redirect(current_url());
				}
				
				$dial_code = $this->Igain_model->get_dial_code1($data['Country_id']);
				
				$Membership_id =  strip_tags($this->input->post('Membership_id'));

				$phnumber = App_string_encrypt($dial_code.$Membership_id);
				
				$result = $this->Igain_model->fetch_enrollment_details1($Company_id,$Membership_id,$Login_Enrollement_id,$phnumber);
				...
				if($Membership_id == $Login_Membership_id)
				{
					$this->session->set_flashdata("transfer","Please Enter Valid Membership ID..!!");
					redirect(current_url());
				}
				...
				$Member_Membership_id = $result1[0]['Card_id']; 
				$Member_Enrollement_id = $result1[0]['Enrollement_id']; 
				$Member_Current_balance = $result1[0]['Current_balance']; 
				
				
				$Transfer_Points= strip_tags($this->input->post("Transfer_Points"));
				
				$checkPoints = is_numeric($Transfer_Points);
				if($checkPoints !=1)
				{
					$this->session->set_flashdata("transfer","Please Enter Valid Transfer Points");
					redirect(current_url());
				}
				if($Transfer_Points > $Current_point_balance)
				{
					$this->session->set_flashdata("transfer","Insufficient Current Balance");
					redirect(current_url());
				}
				...
				else
				{
					...
					if($result > 0 )
					{
						...
						$Email_content=array(
							...
							$this->send_notification->send_Notification_email($Login_Enrollement_id,$Email_content,$seller_id,$Company_id);
						...
						$Email_content12=array(
							...
							$this->send_notification->send_Notification_email($Member_Enrollement_id,$Email_content12,$seller_id,$Company_id);
		...
		function getsurveyquestion() //from mail or Notification
	{
		...
		{	
			...
			$data['userId']= $session_data['userId'];
			$Card_id= $session_data['Card_id'];
			$gv_log_compid= $session_data['Company_id'];
			$data['timezone_entry']= $session_data['timezone_entry'];
			// var_dump($_POST);
			// die;App_string_encrypt
			// error_reporting(0);
			...
			// $survey_id = $Survey_data['Survey_id'];Survey_data
			
			// echo '<br>From_survey_mail '.$From_survey_mail;die;
			// echo '<br>survey_id '.$survey_id;die;From_survey_mail
			if(is_numeric($survey_id)!=1 || is_numeric($smartphone_flag)!=1  || is_numeric($From_survey_mail)!=1)
			{
				redirect('Cust_home/getsurvey');
			}
			// echo '<br>survey_id '.$survey_id;die;
	...
	function getsurveyquestionWEB() //from mail or Notification
	{
		if($this->session->userdata('cust_logged_in'))
		{	
			$session_data = $this->session->userdata('cust_logged_in');
			...
			if(is_numeric($_REQUEST['Survey_data'])!=1)
			{
				redirect('Cust_home/getsurvey');
			}
			$survey_id = strip_tags($_REQUEST['Survey_data']);
			// echo $survey_id;die;
			$smartphone_flag = 2;
			if($survey_id !="") //from Email
			{
				// echo"--if--condition---<br>";From_survey_mail
				$data['Survey_details'] = $this->Survey_model->fetch_survey_questions($survey_id,$gv_log_compid,$Enrollment_id);
				if($data['Survey_details'] != NULL)
				{
					...
					// die;
						...
						// echo"---sssd-SurveyTemplate----".$SurveyTemplate->Survey_template."--<br>";
						if($SurveyTemplate->Survey_template==1)
						{
							$this->load->view('home/survey_template1/index', $data);
						}
	...
	function enroll_new_member_website()
	{
		...
		$email = App_string_encrypt(strip_tags($_REQUEST['userEmailId']));
		$Company_id = App_string_decrypt($_REQUEST['Company_id']);
		$Country = App_string_decrypt($_REQUEST['Country']);
		$flag = $_REQUEST['flag'];
		$Phone_no = strip_tags($_REQUEST['phno']);
			...
			if(is_numeric($Phone_no) != 1)
			{
				$this->session->set_flashdata("error_code","Incorrect Mobile No.");
				redirect('login', 'refresh');	
			}
						
			if (!filter_var($_REQUEST['userEmailId'], FILTER_VALIDATE_EMAIL)) {
			  $this->session->set_flashdata("error_code","Invalid email format");
				redirect('login', 'refresh');	
			}
			
			
			if(is_numeric($Company_id) != 1 || is_numeric($Country) != 1 )
			{
				$this->session->set_flashdata("error_code","Incorrect Data");
				redirect('login', 'refresh');	
			}
			
		if($first_name !="" && $last_name !="" && $_REQUEST['userEmailId'] !="" && $Company_id !="" && $Country !=""  )
		{
			...
			echo '<br>email '.$email;
			echo '<br>email2 '.strip_tags($_REQUEST['userEmailId']);
			...
			$dialcode=$Dial_Code->phonecode;
			$phoneNo=App_string_encrypt($dialcode.''.$Phone_no);
			$Pass=App_string_encrypt($Phone_no);
			$timezone_entry=$Super_Seller_details->timezone_entry;
				...
				if($CheckPhone_number ==0)
				{
					...
					/**********************igain Log Table change 29-06-2017 *************************/
						...
						if($Last_enroll_id > 0)
						{
							...
							$post_data=array('Pwd_set_code' => $Set_pwd_code);
							$result2 = $this->Igain_model->update_password($Company_id,$Last_enroll_id,$post_data);
							...
							$this->session->set_flashdata("success_code","Your Enrollment is Successful! Login credentials are sent to your registered Email Id !!");

						}
						else
						{
							$this->session->set_flashdata("error_code","Entered Mobile No. already exist...Please Enter valid Mobile No.!!");
						}
				}
				else
				{
			...
			{
				$this->session->set_flashdata("error_code","Entered Email ID already exist...Please Enter valid Email ID");
		...
		{
			$this->session->set_flashdata("error_code","Please Enter all the fields !!!");
			redirect('login', 'refresh');
	...
	function update_hobbies()
	{
		if($this->session->userdata('cust_logged_in'))
		{
			...
			{
				$this->session->set_flashdata("error_code","Please Select Hobbies !!");
			}
		}
		else
		{
