<?php 
error_reporting(0);
class Merchant_api_test extends CI_Controller 
{
	...
	public function Novacom_api()
	{
		...
		// print_r($headers);
		// echo "<br>";
		$Brand_id = $headers['Brand-Id'];
		$Outlet_id = $headers['Outlet-Id'];
		$API_flag = $headers['Api-Flag'];
		$Company_username = $headers['Company-Username'];
		$Company_password = $headers['Company-Password'];


		echo "Brand_id--".$Brand_id."<br>";
		echo "Outlet_id--".$Outlet_id."<br>";
		echo "API_flag--".$API_flag."<br>";
		echo "Company_username--".$Company_username."<br>";
		echo "Company_password--".$Company_password."<br>";
		exit;
		if($getHeaders!=Null)
		{
									...
									if($Voucher_result != Null)
									{
										...
										{	
											...
											if($Product_Voucher_id ==0) // product voucher in percentage
											{
												...
												{
													foreach($Pos_Item_details_array as $item_array)
													{
														$Item_Num1 = $item_array["Item_Num"];
														$Item_Quantity1 = $item_array["Quantity"];
														
														if($Product_Voucher_item_code == $Item_Num1)
														{
															$Free_Item_Num = $Item_Num1;
															$Free_Item_Qty = $Item_Quantity1;
														}
													}
													$ProductInPercentageVouchers= $this->Merchant_api_model->get_member_product_inprecentage_vouchers($Card_id,$Company_id,$Free_Item_Num,$Free_Item_Qty,$Gift_card_id);
													
													if($ProductInPercentageVouchers != Null)
													{
														$Reduce_product_amt = $ProductInPercentageVouchers->Reduce_product_amt;
										...
										{	
											...
											if($Product_Voucher_id ==0) // product voucher in percentage
											{
												...
												if(in_array($Product_Voucher_item_code, $Cust_Item_Num))
												{
													...
													$ProductInPercentageVouchers= $this->Merchant_api_model->get_member_product_inprecentage_vouchers($CardId,$Company_id,$Free_Item_Num,$Free_Item_Qty,$Pos_voucher_no);
												...
												{ 
													...
													{		
														...
														{	
															...
															{
																$Total_sent_voucher = $this->Merchant_api_model->member_sent_offers($Company_id,$Customer_enroll_id,$offer['Offer_code']);
															...
															//****now send product voucher notification *****************
																...
																$Email_content76 = array
																	( 
																		...
																		'Description' => "Present above voucher code during your next purchase and get ".$offer['Free_item']." ".$FreeItem->Merchandize_item_name." free",
							...
							/********************************************************************************/
									...
									if($Error_Records != NULL)
									{
									  ...
									  $pos_html = "<p>Hello,</p>
										 ...
										 $this->email->bcc(array('rakesh@miraclecartes.com','amit@novacom.co.ke'));
