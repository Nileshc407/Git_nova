...
class Merchant_api extends CI_Controller 
{
	...
	public function Novacom_api()
	{
		...
		if($getHeaders!= NULL)
		{
			$API_flag = $getHeaders['Api-Flag'];	
			$API_Company_username = $getHeaders['Company-Username'];	
			$API_Company_password = $getHeaders['Company-Password'];	
		}
		...
		else
		{
			...
			if($Check_company_exist !=NULL)
			{
				...
				$Decrypt_Company_password = App_string_decrypt($API_Company_password);
				
				$Api_url = base_url();
				if($Api_url == "https://ehp.online/")
				{
					$Server_name = "LIVE";
				}
				else if($Api_url == "http://novacomonline.ehpdemo.online/")
				{
					$Server_name = "DEMO";
				}
				else
				...
				if($Password_match == 0)
				{
					$Company_details = $this->Merchant_api_model->get_company_details($Company_id);

					$Company_id = $Company_details->Company_id;
					$Allow_merchant_pin = $Company_details->Allow_merchant_pin;
					$Allow_member_pin = $Company_details->Pin_no_applicable;
					
					$Login_Redemptionratio=$Company_details->Redemptionratio;
					$Country_id=$Company_details->Country;
					$Company_name1=$Company_details->Company_name;
					...
					$Gift_card_validity_days=$Company_details->Gift_card_validity_days;
					$Company_redeem_request_validity = $Company_details->Redeem_request_validity; 
					$Stamp_voucher_validity = $Company_details->Stamp_voucher_validity;
					
					$Company_meal_flag = $Company_details->Enable_company_meal_flag;
					$Redeem_auto_confirm = $Company_details->Redeem_auto_confirm_flag;
					$Use_points_as_discounts = $Company_details->Points_as_discount_flag;
					$Company_discount_code = $Company_details->Discount_code;
					
					$Use_Voucher_as_discounts = $Company_details->Voucher_as_discount_flag;
					$Company_voucher_discount_code = $Company_details->Voucher_discount_code;
					$Daily_points_consumption_flag = $Company_details->Daily_points_consumption_flag;  
					$Member_based_earning_flag = $Company_details->Member_based_earning_flag;  
					$Member_min_points_flag = $Company_details->Min_points_flag;  
					$Use_member_pin_number_as_card_id = $Company_details->Use_pin_number_as_card_id;  
					$Member_based_redeem_flag = $Company_details->Member_based_redeem_flag;  
					$Company_card_decsion = $Company_details->card_decsion;  
					$Company_next_card_no = $Company_details->next_card_no;  	
					$Joining_bonus_flag = $Company_details->Joining_bonus;
					$Joining_bonus_points = $Company_details->Joining_bonus_points;					
					$Physical_card_link = $Company_details->Physical_card_link;					
						
					// $API_flag2 = $this->string_decrypt($API_flag, $key, $iv);
					// $API_flag = preg_replace("/[^(\x20-\x7f)]*/s", "", $API_flag2);
					$API_flag = App_string_decrypt($API_flag);
				/***************************Fetch Member Info**************************/
					if($API_flag == 20)  //Fetch_Member_info
					{
						...
						if($result!=NULL)
						{
							...
							$Current_address = App_string_decrypt($result->Current_address);
							$Zipcode = $result->Zipcode;
							
							$Phone_no = App_string_decrypt($result->Phone_no);
							$exp=explode($dial_code,$Phone_no);
							$Mobile_no = $exp[1];
							...
							{
								$result96 = $this->Merchant_api_model->Get_Member_Tier_Details($lv_member_Tier_id,$Company_id);
								$Tier_consumption_limit = $result96->Redeemtion_limit;  
								if($Tier_consumption_limit > 0)
								{
									$ConsumptionDetails = $this->Merchant_api_model->Get_points_consumption_details($Cust_Enrollement_id,$Company_id);
									if($ConsumptionDetails !=Null)
									{
										$Total_points_used = $ConsumptionDetails->Total_points_used;
									}
									else
									{
										$Total_points_used = 0;
									...
									$Daily_Available_Balance = $Tier_consumption_limit - $Total_points_used;
									
									$Net_balance = $Current_point_balance - $Daily_Available_Balance;
									if($Net_balance < 0)
									{
										$Daily_Available_Balance = $Current_point_balance;
									}
									else
									{
										$Daily_Available_Balance = $Daily_Available_Balance;
									}
									
									$Current_point_balance = $Daily_Available_Balance;
								}
							}
							
							$get_city_state_country = $this->Merchant_api_model->Fetch_city_state_country($Company_id,$Cust_Enrollement_id);
							if($get_city_state_country != Null)
							{
								$Current_address= App_string_decrypt($get_city_state_country->Current_address);
								...
								$Zipcode=$get_city_state_country->Zipcode;
								
								$Address_details = array("Address"=>$Current_address,"Country"=>$Country_name,"State"=>$State_name,"City"=>$City_name,"Zipcode"=>$Zipcode);
							}
							else
							{
								$Address_details = array("Address"=>$Current_address,"Country"=>Null,"State"=>Null,"City"=>Null,"Zipcode"=>$Zipcode);
							}
					...
					if($API_flag == 75)
					{
						...
						if($result!=NULL)
						{
							...
							$Current_point_balance = $Current_balance-($Blocked_points+$Debit_points);
							$Current_address = App_string_decrypt($result->Current_address);
							$Alternative_address = App_string_decrypt($result->Alternative_address);
							$Email_id = App_string_decrypt($result->User_email_id);
							$Zipcode = $result->Zipcode;
							$Card_id = $result->Card_id;
							$Physical_card_id = $result->Card_id2;
							...
							$Customer_details = array(
									...
									"Membership_id" => $Membership_id,
									"Card_id" => $Card_id,
									"Customer_id" => $Physical_card_id,
									"Customer_name" => $Memeber_name,
									"Customer_email" => $Email_id,
									"Customer_phone" => $Mobile_no,
									"Customer_address" => $Current_address,
					...
					{	
						...
						$Gift_redeem_by = 0;
						$balance_to_pay = '';	
						$Pin_type = 0;	// redeem points	
						
						$dial_code = $this->Igain_model->get_dial_code($Country_id); //Get Country Dial code	
						$phnumber = $dial_code.$Membership_id;
												
						$result = $this->Merchant_api_model->get_pos_redeem($Membership_id,$Company_id,$phnumber,$Pin_type); //Get Customer details
						if($result!=NULL)
						{
							if($result == 3111)
							{
							...
							if($Member_based_redeem_flag == 1)
							{
								$redeem_result = $this->Merchant_api_model->get_redeem_code_details($Company_id,$Cust_enrollement_id,$Card_id);
								if($redeem_result != Null)
								{
									$Cust_redeem_points = $redeem_result->Redeem_points;
									
									$Redeem_points = $Cust_redeem_points;
									
									$Cust_redeem_code = $redeem_result->Pin;
									
									if($Cust_redeem_code != $Membership_id)
									{
										$Result1322 = array(
													"Error_flag" => 2003,
													"Message" => "Invalid Code Type, Please Generate Correct Code",
													"Membership_id" => null,
													"Member_name" => null,
													"Current_points" => 0,
													"Redeem_points" => 0,
											...
											echo json_encode($Result1322);
											exit;	
									}
									if($Cust_redeem_points != $Redeem_points)
									{
										$Result3104 = array(
												"Error_flag" => 3104,
												"Message" => "Redeem points doesnt match with Customer",
												"Membership_id" => $Membership_id,
												"Member_name" => $Memeber_name,
							...
							{
								if($Member_min_points_flag == 1)
								{
									$Total_reddems = 0; //check Tier_redeemtion_limit for every transaction
								}
								if($Total_reddems == 0)
								{
							...
							if($Daily_points_consumption_flag == 1)
							{
								$result52 = $this->Merchant_api_model->Get_Member_Tier_Details($lv_member_Tier_id,$Company_id);
								$Tier_consumption_limit = $result52->Redeemtion_limit;
								
								$Consumption_details = $this->Merchant_api_model->Get_points_consumption_details($Cust_enrollement_id,$Company_id);
							...
							if($result1!=NULL)
							{
								...
								if($result2!=NULL)
								{
									...
									{	
										$result221 = $this->Merchant_api_model->Check_redeem_request_issent($Cust_enrollement_id,$Company_id,$Seller_id,$Redeem_points,$Pos_bill_no,$ChannelCompanyId);
										...
										if($result221 != Null)
										{
											...
											if($Earlier_request_flag == 0)
											{
												...
												if($currentTime > $cenvertedTime)
												{
													...
													$this->Merchant_api_model->update_cust_old_redeem_request($data1,$Cust_enrollement_id,$Company_id,$Seller_id,$ChannelCompanyId);
												}
											}
										}
										
										$result22 = $this->Merchant_api_model->Check_redeem_request_issent($Cust_enrollement_id,$Company_id,$Seller_id,$Redeem_points,$Pos_bill_no,$ChannelCompanyId);
										...
										$result2 = $this->Merchant_api_model->Get_cust_approved_request($Cust_enrollement_id,$Company_id,$Seller_id,$Redeem_points,$Pos_bill_no,$ChannelCompanyId);
					...
					if($API_flag == 87) // new development apply discount rule for artcaffe
					{
						...
						$Bill_Total = $inputArgs['Bill_Total']; //POS Outlet
								...
								if($result!=NULL)
								{
									if($result == 3111)
									{
										$Result1311 = array("Error_flag" => 3111, 
															...
															"Balance_due" => number_format($Bill_Total,2),
									...
									if($Bill_Total < $Pos_discount)
									{
										$Result1002 = array("Error_flag" => 1002, 
															"Message" =>'Invalid bill Or Order amount',
															"Membership_id" => $Membership_id,
															"Card_id" => $Card_id,
															"Member_name" => $Memeber_name,
															"Bill_total" => number_format($Bill_Total,2),
									...
									{ 
										...
										/*******************get item details*******************/
											...
											if($ItemDetails !=NULL)
											{
												...
												$Itemcategory_price[$Itemcategory_id] = $Item_price; // 25-05-23 nilesh category discount
												/***************11-7-2020*************/
											}
											else
											{
							...
							if($result1!=NULL)
							{
								...
								{	
									if($result == 3111)
									{
										if($Use_Voucher_as_discounts == 1)
										{
											$Result1311 = array("Error_flag" => 3111, 
																...
																"Balance_due" => number_format($Bill_amount,2),
																"Voucher_apply" => "D",
																"Discount_code" => $Company_voucher_discount_code,
																"Discount_amt" => number_format(0,2)
																);
											echo json_encode($Result1311);
											exit;	
									...
									if($Voucher_result != Null)
									{
										...
										$Used_flag = $Voucher_result->Used_flag;
										...
										{	
											...
											if($Product_Voucher_id ==0) // product voucher in percentage
											{
												...
												{ 
													...
													if($ItemDetails !=NULL)
													{
														...
														$CheckItemTempCart = $this->Merchant_api_model->GetItemsDetails($Company_id,$Cust_enrollement_id,$ItemCode,$Outlet_no,$ChannelCompanyId  );
														...
														else
														{
															...
															$this->Merchant_api_model->insert_item($data79);
														}
												...
												$GetItems= $this->Merchant_api_model->Get_items($Company_id,$Cust_enrollement_id,$Outlet_no,$ChannelCompanyId);
												if($GetItems != Null)
												{
											...
											/**********************stamp new logic 02-05-2021**************************/
												...
												$delete_temp_cart = $this->Merchant_api_model->delete_pos_temp_cart_data($Company_id,$Cust_enrollement_id,$Outlet_no,$ChannelCompanyId);
												...
												if($ReduceDiscountAmt > 0)
												{
													...
													if($Use_Voucher_as_discounts == 1)
													{
														$Result172 = array("Error_flag" => 1001,
																	...
																	"Voucher_apply" => "D",
																	"Discount_code" => $Company_voucher_discount_code,
																	"Discount_amt" => number_format($Reduce_product_amt,2)
																	);
											...
											else // sent product voucher 
											{
												...
												$Product_voucher_items = array();
												
												$Sent_Product_Voucher = $this->Merchant_api_model->Get_Sent_Product_Voucher_Details($Gift_card_id,$Cust_enrollement_id,$Company_id);
												
												if($Sent_Product_Voucher != Null)
												{
													foreach($Sent_Product_Voucher as $Pvoucher)
													{
														$Product_voucher_items[] = $Pvoucher['Company_merchandize_item_code'];
													}
												}
												
												$Intersection = array_intersect($Cust_Item_Num, $Product_voucher_items);
										...
										if($Used_flag == 0)
										{
											...
											if($Use_Voucher_as_discounts == 1)
											{
												$Result172 = array("Error_flag" => 1001,
																	...
																	"Voucher_apply" => "D",
																	"Discount_code" => $Company_voucher_discount_code,
																	"Discount_amt" => number_format($Card_balance,2)
																	);
												echo json_encode($Result172);
												exit;	
											}
					...
					{ 
						...
						if($Pos_trans_type ==1)
						{
							...
							$Customer_code = $cardId;
							
							$Pin_type = 1; // earn point
							...
							$cust_details = $this->Merchant_api_model->cust_details_from_card_earn($Company_id,$cardId,$phnumber,$Pin_type); //Get Customer details
							if($cust_details !=NULL)
							{
								...
								$Total_reddems_check = $Total_reddems;
								
								if($Member_min_points_flag == 1)
								{
									$Total_reddems_check = 0; //check Tier_redeemtion_limit for every transaction
								}
								if($Pos_points_redeemed >0)
								{
									if($Total_reddems_check == 0)
									{
										$result51 = $this->Merchant_api_model->Get_Member_Tier_Details($lv_member_Tier_id,$Company_id);
										$Tier_redeemtion_limit = $result51->Redeemtion_limit;
										if($Tier_redeemtion_limit != Null)
										{
											if($Current_bal1 < $Tier_redeemtion_limit)
											{
												$Result12 = array(
															"Error_flag" => 3101,
															"Message" => "Insufficient Points Balance",
							...
							if($Pos_bill_items != Null)
							{
								...
								{ 
									...
									if($ItemDetails !=NULL)
									{
										...
										// $ItemCodeArr[$ItemCode]=$item['Item_Qty'];
										$CheckItemTempCart = $this->Merchant_api_model->GetItemsDetails($Company_id,$Cust_enrollement_id,$ItemCode,$delivery_outlet,$ChannelCompanyId);
										if($CheckItemTempCart != Null)
										{
											$TempQty = $CheckItemTempCart->Item_qty;
										...
										else
										{
											...
											$this->Merchant_api_model->insert_item($data78);
										}
									}
									else
									{
										$delete_temp_cart = $this->Merchant_api_model->delete_pos_temp_cart_data($Company_id,$Cust_enrollement_id,$delivery_outlet,$ChannelCompanyId);
										$ResponseData = array("Error_flag" => 3103,
															  "Message" => "Invalid Item_Num Or Item not exist.",
															  "Item_Num" => $ItemCode,
															  "Membership_id" => $CardId,
							...
							$Pos_bill_items = $this->Merchant_api_model->Get_temp_cart_items($Company_id,$Cust_enrollement_id,$delivery_outlet,$ChannelCompanyId);
							/*****************check bill amount and item amount 27-09-2021***********************/	
								$Bill_total_array = array();
									...
									else
									{
										...
										{	
											...
											if($Product_Voucher_id ==0) // product voucher in percentage
											{
												...
												{ 
													...
													if($ItemDetails !=NULL)
													{
														...
														$ItemCodeArr[$ItemCode]=$item['Item_Qty'];
														/* $CheckItemTempCart = $this->Merchant_api_model->GetItemsDetails($Company_id,$Cust_enrollement_id,$ItemCode);
														if($CheckItemTempCart != Null)
														{
															$TempQty = $CheckItemTempCart->Item_qty;
															...
															$this->db->where(array("Company_id"=>$Company_id,"Enrollment_id"=>$Cust_enrollement_id,"Item_code"=>$ItemCode));
															$this->db->update("igain_pos_temp_cart",$TempCartData);
														}
														else
														{
															...
															$data78['Item_code'] = $ItemCode;
															$data78['Item_qty'] = $item['Item_Qty'];
															$data78['Item_price'] = $item['Item_Rate'];
												...
												/* $GetItems= $this->Merchant_api_model->Get_items($Company_id,$Cust_enrollement_id);
												if($GetItems != Null)
												{
													foreach($GetItems as $row1)
													{
											...
											{	
												...
												$Sent_Product_Voucher = $this->Merchant_api_model->Get_Sent_Product_Voucher_Details($Pos_voucher_no,$Customer_enroll_id,$Company_id);
							...
							/********************09-04-2021 validate gift card**********************/
								if($Pos_giftcard_no)
								{
									$Giftcard_result = $this->Merchant_api_model->Validate_gift_card($Company_id,$Pos_giftcard_no);
							...
							/********************09-04-2021 validate gift card**********************/
								...
								if($result!=NULL)
								{
									...
									// $grand_total = $grand_total - $Pos_giftcard_amount // 09-04-2021
								...
								/*	
									...
									if($grand_total < 0 )
									{
										...
										/*************************************/
											$Seller_result = $this->Pos_model->Get_Seller_details($seller_id,$Company_id);	
											...
											// $str = substr($tp_db,0,5);
											// $bill = substr($tp_db,5,$len);
											$bill1 = substr($tp_db,5,$len);
											
											$bill_result = $this->Merchant_api_model->check_bill_no($bill1,$Company_id);
										...
										$Extra_earn_points_Loyalty_pts = array();
											foreach ($Pos_bill_items as $item)
											{
																...
																{											
																	...
																	foreach($Get_segments2 as $Get_segments)
																	{
																		...
																		if($Get_segments->Segment_type_id==10)//Cumulative Points Accumulated
																		{
																			...
																			$Trans_Records = $this->Pos_model->get_cust_trans_summary_all($Company_id,$Customer_enroll_id,$start_date,$end_date,$transaction_type_id,$Tier_id,'','');
																		...
																		if($Get_segments->Segment_type_id==11)//Single Transaction  Amount
																		{
																			...
																			$Trans_Records = $this->Pos_model->get_cust_trans_details($Company_id,$start_date,$end_date,$Customer_enroll_id,$transaction_type_id,$Tier_id,'','');
																			foreach($Trans_Records as $Trans_Records)
																			{
												...
												//***********18-04-2020-allow to redeem 1 point extra****************/
													...
													if (is_nan($PaidAmount1)) 
													{
														$PaidAmount1 = 0;
													}
													if (is_nan($Weighted_redeem_points1)) 
													{
														$Weighted_redeem_points1 = 0;
													}
													if (is_nan($Weighted_Redeem_amount)) 
													{
														$Weighted_Redeem_amount = 0;
													}
													if (is_nan($Weighted_loyalty_points)) 
													{
												...
												/***************Update gift card and vouchers 23-2-2021********************/
													...
													if($redeemed_discount_voucher != Null)
													{
														...
														$Voucher_array = ["voucher_no"=>$redeemed_discount_voucher,"amount"=>$Pos_voucher_amount];
												...
												/***********09-04-2021 update gift card******************/
													...
													{
														$Voucher_array = ["voucher_no"=>$redeemed_giftcard,"amount"=>$Pos_giftcard_amount];
										...
										/************* Update Current Balance ******************/
											...
											if($billcount == 1)
											{
												$seller_id = $Pos_outlet_id1;
											}
											$result4 = $this->Pos_model->update_billno($seller_id,$billno_withyear);
										/*********** Update Current Balance ***************/
										
										/************************************/
											$updateData2 = array('Available_balance' => $Update_Current_balance);
												
											$this->db->where(array('Trans_type' => 2,'Bill_no' => $bill,'Card_id' => $cid,'Company_id' => $Company_id));
											$this->db->update("igain_transaction",$updateData2);
										/************************************/
											if($Use_member_pin_number_as_card_id == 1)
											{
												$CustomerCodeData = array(
													'Outlet_id' => $Pos_outlet_id1,
													'Pos_bill_no' => $Pos_bill_no,
													'pinno_used' => 1
													);
												$this->Pos_model->update_customer_earn_code($Customer_code,$Company_id,$cid,$CustomerCodeData);
											}   
										/************************************/
										$lvp_date_time = date("Y-m-d H:i:s");
										$Email_content = array
										(
											...
											'DiscountAmt' => $Pos_discount_amount-$Pos_voucher_amount,
								...
								//****************06-09-21*********************
									if($Daily_points_consumption_flag == 1)
									{
										...
										if($Tier_consumption_limit > 0)
										{
											...
											$Net_balance = $Available_Balance - $Daily_Available_Balance;
											if($Net_balance < 0)
											{
												$Daily_Available_Balance = $Available_Balance;
											}
											else
											{
												$Daily_Available_Balance = $Daily_Available_Balance;
											...
											$Available_Balance = $Daily_Available_Balance;
										}
									}
								//****************06-09-21*********************
					...
					{ 
						$cardId = $inputArgs['Membership_id'];
						$Pos_outlet_id = $inputArgs['Outlet_No'];
						$Outlet_Name = $inputArgs['Outlet_Name'];
						$Pos_bill_no = $inputArgs['Bill_No'];
						$Pos_bill_date_time = $inputArgs['Bill_Date_Time'];
						$Email_id = $inputArgs['Email_id'];
						$Phone_No = $inputArgs['Phone_No'];
						$Bill_Status = $inputArgs['Bill_Status'];
						$Bill_Total = $inputArgs['Bill_Total'];
						$Bill_Items = $inputArgs['Bill_Items'];
						$Redeem_Details = $inputArgs['Redeem_Details'];
						$Points_Redeemed = $Redeem_Details['Points_Redeemed'];
						$Points_Amount = $Redeem_Details['Points_Amount'];
						$Bill_Tenders = $inputArgs['Bill_Tenders'];
						
						$Bill_Total = str_replace( ',', '', $Bill_Total);
						$Points_Redeemed = str_replace( ',', '', $Points_Redeemed);
						$Points_Amount = str_replace( ',', '', $Points_Amount);
						...
						if($cust_details !=NULL)
						{
							...
							if($Current_bal1 <$Points_Redeemed)
							{
								$Result12 = array(
													"Error_flag" => 3101,
							...
							$BalanceDue = $Bill_Total - $Points_Amount;
							if($BalanceDue < 0 )
							{
								$Result1327 = array("Error_flag" => 2066,
													"Bill_total" => number_format($Bill_Total,2),
													"Points_amount" => number_format($Points_Amount,2),
													"Message" => "Points amount is more than bill total."
													); 
								echo json_encode($Result1327); 
								exit;
							}
							...
							if($result!=NULL)
							{
								...
								if($Bill_Items != Null)
								{
									...
									{ 
										...
										for($i=1;$i<=$ItemQty;$i++)
										{
											$Weighted_Points_Redeemed = ($Points_Redeemed * $ItemRate) / $Bill_Total;
											$Weighted_Points_Amount = ($Points_Amount * $ItemRate) / $Bill_Total;
											$Paid_amount = $ItemRate - $Weighted_Points_Amount;
											
											$gift_cardid = $this->Merchant_api_model->getVoucher();
											
											$giftData["Company_id"] = $Company_id;
											$giftData["Gift_card_id"] = $gift_cardid;
											$giftData["Card_balance"] = $ItemRate;
											$giftData["Card_value"] = $ItemRate;
											$giftData["Card_id"] = $CardId;
											...
											$data123 = array('Company_id' => $Company_id,
												'Trans_type' => $Trans_type, 
												'Purchase_amount' => $ItemRate,
												'Paid_amount' => $Paid_amount,
												'COD_Amount' => $Paid_amount,
												'Redeem_points' => $Weighted_Points_Redeemed,
												'Redeem_amount' => $Weighted_Points_Amount,
												'balance_to_pay' => $Paid_amount,
												'Payment_type_id' => $Payment_type_id,
												'Remarks' => $Remarks,
												'Trans_date' => $lv_date_time,
												'Enrollement_id' => $Customer_enroll_id,
											...
											$GiftCard = array("Item_Num" => $ItemNum,"Item_Name" =>$temName,"Item_Qty" => $ItemQty, "Item_Rate" => $ItemRate,"GiftCardNo" => $gift_cardid, "Valid_till" => date("d M Y", strtotime($Valid_till)));
											
											$Gift_card_array[] = $GiftCard;
										}
									}	
									...
									$Email_content = array(
											  ...
											  'Symbol_currency' => $Symbol_currency,
											  'Outlet_name' => $Pos_outlet_name,
											  'Gift_card_array' => $Gift_card_array,
									...
									$Update_Current_balance = $bal - $Points_Redeemed;
									$Update_total_reddems = $Total_reddems + $Points_Redeemed;
									
									$up = array('Current_balance' => $Update_Current_balance,'Total_reddems' => $Update_total_reddems,'pinno_used' => 1);
										
									$this->Pos_model->update_cust_balance($up,$CardId,$Company_id);	
									
									$CustomerEmail = App_string_decrypt($CustomerEmail);
									$Balance_due = $Bill_Total - $Points_Amount;
									
									$GiftCardDetails = array(
													"Error_flag" => 1001,
													"Bill_total" => number_format($Bill_Total,2),
													"Giftcard_amount" => number_format($Bill_Total,2),
													"Balance_due" => number_format($Balance_due,2),
													"Membership_id" => $CardId,
													"Member_name" => $CustomerName,
													"Email_id" => $CustomerEmail,
													"Bill_note" => "Gift Card Sent on Email"
												);
									/******************22-3-2021 insert JSON*****************/
										$inputArgs1 = json_encode($inputArgs);
										$outputArgs1 = json_encode($GiftCardDetails);
						...
						{
							$cardId = $Company_id.'999999999999999'; // walking customer card id
							$cust_details = $this->Merchant_api_model->cust_details_from_card($Company_id,$cardId,$phnumber); //Get Customer details
							if($cust_details !=NULL)
							{
								foreach($cust_details as $row25)
							...
							else
							{
								$Result127 = array(
												"Error_flag" => 3113,
												"Message" => "Walkin customer not found in system please register Walkin customer..!"
											);
								...
								if($Bill_Items != Null)
								{
									...
									{ 
										...
										for($i=1;$i<=$ItemQty;$i++)
										{
											...
											$giftData["Phone_no"] = App_string_encrypt($CustomerPhone);
											$giftData["Payment_Type_id"] = $Payment_type_id; 
											$giftData["Seller_id"] = $Pos_outlet_id;
											$giftData["Valid_till"] = $Valid_till;
									...
									$Email_content = array(
											  ...
											  'Balance_due' => $BalanceDue,
											  'Notification_type' => 'Gift Card',
											  'Template_type' => 'Purchase_gift_card_walking'
										  );
									$GiftNotification=$this->send_notification->send_Notification_email($Customer_enroll_id,$Email_content,$Pos_outlet_id,$Company_id);  
					...
					/*******************Fetch Merchant Profile***********************/
					/*********************Update Customer Profile Details from Pos****************************/
					if($API_flag == 5)
					{	
						$Membership_id = $inputArgs['Membership_id']; 
						$Customer_name = $inputArgs['Customer_Name']; 
						$Customer_email_id = $inputArgs['Customer_Email'];
						$Customer_phone = $inputArgs['Customer_Phone']; 
						$Customer_address = $inputArgs['Customer_Address']; 
						$Alternative_address = $inputArgs['Alternative_Address'];
						
						if($Customer_name =="" || $Customer_email_id =="" || $Customer_phone =="" || $Customer_address =="")
						{
							$Result404 = array(
												"Error_flag" => 404,
						...
						if($result!=NULL)
						{
							...
							if($Customer_email_id != $Email_id)
							{
								$Check_email_id = App_string_encrypt($Customer_email_id);
								
								$Check_EmailID=$this->Igain_model->Check_EmailID($Check_email_id,$Company_id);
								if($Check_EmailID != 0)
								{
									$Result2030 = array(
														"Error_flag" => 2030,
								...
								{
									$post_data['User_email_id'] = $Check_email_id;
								}
							}
							if($Customer_phone != $Mobile_no)
							{
								$phnumber = $dial_code.$Customer_phone;
								
								$Check_phnumber = App_string_encrypt($phnumber);
								
								$CheckPhone_number=$this->Igain_model->CheckPhone_number($Check_phnumber,$Company_id);
								...
								{
									$post_data['Phone_no'] = $Check_phnumber;
								}
							}
							$strname = explode(" ", $Customer_name);
							$first_name = $strname[0];
							$last_name = $strname[1];
							$post_data['First_name'] = $first_name;
							$post_data['Last_name'] = $last_name;
							$post_data['Current_address'] = App_string_encrypt($Customer_address);
							$post_data['Alternative_address'] = App_string_encrypt($Alternative_address);
								
							
							$result222 = $this->Enroll_model->update_enrollment($post_data,$Cust_Enrollement_id);
							...
							$Alternative_address = App_string_decrypt($Enroll_details->Alternative_address);
							$Email_id = App_string_decrypt($Enroll_details->User_email_id);
							
							$Card_id = $Enroll_details->Card_id;
							$Physical_card_id = $Enroll_details->Card_id2;
						...
						else
						{
							...
							echo json_encode($Result2003); //Unable to Locate membership id
							exit;
						}
					}
					/****************************Update Customer Profile Details From Pos*****************************************/
					...
					/***********************Fetch Merchant info*********************/ 
					/*********************Quick Enrollment***************************/ 
					if($API_flag == 6) // Customer Enrollment from Pos 
					{ 
						$Chk_card = $inputArgs['Membership_id'];
						$Membership_id = $inputArgs['Membership_id']; 
						$Customer_name = $inputArgs['Customer_Name']; 
						$Customer_email_id = $inputArgs['Customer_Email'];
						$Customer_phone = $inputArgs['Customer_Phone']; 
						...
						$result123 = $this->Igain_model->Fetch_Super_Seller_details($Company_id);				   
						...
						{											
							...
							$Country_id = $result123->Country_id;
							$timezone_entry = $result123->timezone_entry;
							$Topup_Bill_no = $result123->Topup_Bill_no;

							$tp_db = $Topup_Bill_no;
							$len = strlen($tp_db);
							$str = substr($tp_db,0,5);
							$tp_bill = substr($tp_db,5,$len);
							
							$Dial_Code = $this->Igain_model->get_dial_code11($result123->Country);
							$dialcode=$Dial_Code->phonecode;
							$phoneNo=$dialcode.''.$Customer_phone;
							...
							$CheckPhone_number=$this->Igain_model->CheckPhone_number($Phone_no,$Company_id);
							if($CheckPhone_number ==0)
							{
								$email_id = App_string_encrypt($Customer_email_id);
								
								$Check_EmailID=$this->Igain_model->Check_EmailID($email_id,$Company_id);
								if($Check_EmailID == 0)
								{
									if($Company_card_decsion == 1)
									{
										$next_card_no = $Company_next_card_no;
									}
									if($Physical_card_link == 1)
									{
										if( $Chk_card != "")
										{
											$check_card=$this->Enroll_model->check_card_id2($Chk_card,$Company_id);
											if($check_card > 0)
											{ 
												$Result2029 = array(
															"Error_flag" => 2029,
												...
												echo json_encode($Result2029);
												exit;
											}
											else
											{
												$Physical_card_id = $Chk_card;
											}
										}
									}
								
									if($Joining_bonus_flag == 1 && $next_card_no != Null)
									{									
										$Joining_bonus_points = $Joining_bonus_points;								
									}
									else
									{
										...
										$result = $this->Merchant_api_model->fastenroll($Joining_bonus_points,$Company_id,$Company_card_decsion,$Seller_id,$Country_id,$next_card_no,$timezone_entry,$Customer_name,$email_id,$Phone_no,$Customer_address,$Alternative_address,$Physical_card_id);
										
										$Last_enroll_id=$result;
									
										
										/************ Referee Bonus **************/
										if($Joining_bonus_flag == 1 && $next_card_no != "")
										{
				...
				/**********************3rd Party Loyalty API-Nilesh start22-07-2020********************/
					if($API_flag == 95) // Validate Gift card 
					{
						$Bill_No = $inputArgs['Bill_No']; 
						$Bill_Status = $inputArgs['Bill_Status']; 
						$Bill_amount = $inputArgs['Bill_Total']; 
						$Bill_Date_Time = $inputArgs['Bill_Date_Time']; 
						$Outlet_no = $inputArgs['Outlet_No']; 
						$Outlet_Name = $inputArgs['Outlet_Name']; 
						$Giftcard_Details = $inputArgs['Giftcard_Details']; 
						$Giftcard_No = $Giftcard_Details['Giftcard_No'];	
						...
						/*******************api logic start********************/
							$Giftcard_result = $this->Merchant_api_model->Validate_gift_card($Company_id,$Giftcard_No);
							if($Giftcard_result != Null)
							{
								$Gift_card_id = $Giftcard_result->Gift_card_id;
								$Card_value = $Giftcard_result->Card_value;
								$Card_balance = $Giftcard_result->Card_balance;
								$Valid_till = $Giftcard_result->Valid_till;
								$Card_Payment_Type_id = $Giftcard_result->Payment_Type_id;
								$Discount_percentage = $Giftcard_result->Discount_percentage;
								...
								if($Card_balance > 0)
								{
									...
									$Result172 = array("Error_flag" => 1001,
														"Bill_total" => number_format($Bill_amount,2),
														"Giftcard_no" => $Gift_card_id,
														"Giftcard_amount" => number_format($Card_balance,2),
														"Balance_due" => number_format($Balance_due,2)
														);
									echo json_encode($Result172);
									exit;	
					...
					if($API_flag == 96) //claim the bill and earn points.
					{
						if($Member_based_earning_flag == 1)
						{
							$Pos_bill_no = $inputArgs['Bill_No'];
							$Pos_bill_status = $inputArgs['Bill_Status'];
							$Pos_bill_amount = $inputArgs['Bill_Total'];
							...
							$Pos_bill_amount = str_replace( ',', '', $Pos_bill_amount);
							$PosDiscount = str_replace( ',', '', $Pos_discount);
							$LoyaltyDiscount = str_replace( ',', '', $Pos_loyalty_discount);
							...
							$result01 = $this->Merchant_api_model->check_unclaimed_bill_no($Pos_bill_no,$Pos_outlet_id,$Company_id,$Pos_bill_date_time);				
							if($result01 > 0)
							{
								$result02 = $this->Merchant_api_model->get_unclaimed_bill($Pos_bill_no,$Pos_outlet_id,$Company_id,$Pos_bill_date_time);
								
								$Unclaimed_code = $result02->Claim_code;
								
								$Result4001 = array("Error_flag" => 4001,
														"Message" => "Unclaimed Bill already exists for this store and date",
														"Bill_no" => $Pos_bill_no,
														"Claim_code" => $Unclaimed_code
														); 
								echo json_encode($Result4001); 
								exit;
							}
							
							$result03 = $this->Merchant_api_model->check_claimed_bill_no($Pos_bill_no,$Pos_outlet_id,$Company_id,$Pos_bill_date_time);				
							if($result03 > 0)
							{
								// $result04 = $this->Merchant_api_model->get_claimed_bill($Pos_bill_no,$Pos_outlet_id,$Company_id,$Pos_bill_date_time);
								
								$Result4002 = array("Error_flag" => 4002,
														"Message" => "Claimed Bill already exists for this store and date",
														"Bill_no" => $Pos_bill_no,
														"Claim_code" => ""
														); 
								echo json_encode($Result4002); 
								exit;
							}
							/************************************************************/
							if($Pos_bill_items != Null)
							...
							$Unique_code = $this->Merchant_api_model->get_claim_code($Company_id);
							...
							{ 
								...
								if($ItemDetails !=NULL)
								{
									...
									$data21 = array('Company_id' => $Company_id,
									...
									$Trans_detail = $this->Merchant_api_model->Insert_pos_transaction_temp($data21);
								}
							}
							...
							/******************insert JSON*****************/
								$inputArgs1 = json_encode($inputArgs);
								$outputArgs1 = json_encode($Result1001);
						...
						{
							$Result4003 = array("Error_flag" => 4003,
											"Message" => "Member Based Earning Module Not Enabled! Please Contact The Administrator",
											"Bill_no" => "",
											"Claim_code" => ""
											); 
							echo json_encode($Result4003); 
							exit;
						}	
					}
				...
				/****************************Prestashop purchase api*********************************/ 
					...
					{ 					
							...
							$Seller_id = $getHeaders['Brand'];
							// $Seller_id= $inputArgs['Brand_id'];
							...
							{
								//***************Check outlet exist****************************
								$Outlet_id = $getHeaders['Outlet'];
								// $Outlet_id = $inputArgs['Outlet_id'];
							...
							//---------Delete records from pos error table------------------
							$Delete_pos_api_err = $this->Merchant_api_model->delete_pos_api_error($Company_id,$Seller_id,$Outlet_id);
							//--------------------------------------------------------------
							...
							/********************************************************************************/
									...
									$Error_Records= $this->Merchant_api_model->Select_POS_API_Error_Records($Company_id,$Seller_id,$Outlet_id);
									 if($Error_Records==NULL && $Entry_flag==1)
									{
										$Result1001E = array("Error_flag" => 1001); // Status OK
										$this->output->set_output(json_encode($Result1001E));
										//die;exit;
									...
									else if($Api_url == "https://novacomonline.ehpdemo.online/")
									{
										$Server_name = "DEMO";
									}
									else
									{
									 ...
									 $objWriter->save($_SERVER['DOCUMENT_ROOT'].'/POS_API_ERROR/'.$Server_name.'_POS_API_ERROR-'.$abc.'.xls');
									  // $file_path=($_SERVER['DOCUMENT_ROOT'].'/POS_API_ERROR/'.$Server_name.'_POS_API_ERROR-'.$abc.'.xls');
									  $file_path=('POS_API_ERROR/'.$Server_name.'_POS_API_ERROR-'.$abc.'.xls');
									    ...
									    $pos_html = "<p>Hello,</p>
											...
											$Email_content = array(
											'pos_html' => $pos_html,
											'Pos_error_file' => $file_path,
											'Template_type' => 'Pos_json_error'
										);						
										$send_notification = $this->send_notification->send_Notification_email(0,$Email_content,0,$Company_id);
					...
					{
						$Result404 = array("Error_flag" => 404, 
											"Message" =>'Please Provide Valid API flag');
						echo json_encode($Result404);
						exit;
					}
				}
			...
			else
			{
				$Result129 = array("Error_flag" => 2001,
									"Message" =>'Invalid Company User Name');
				echo json_encode($Result129); // Invalid Company User Name
				exit;
			}
		}
	}
